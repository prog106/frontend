function Star() {
    ...
    let player; // 플레이어를 만들기 위한 동적 몸체 구성 선언.
    let cursor; // 키보드 이벤트를 받기 위한 구성 선언.
    let stars; // 별을 만들기 위한 동적 몸체 구성 선언.
    let bombs; // 폭탄을 만들기 위한 동적 몸체 구성 선언.
    let score = 0; // 점수
    let scoreText; // 점수가 표시
    let gameOver = false;
    const socket = io('http://localhost:3000/star');

    function preload() {
        // 각 이미지 별 변수로 선언
        this.load.image('sky', '/phaser/assets/sky.png'); // 800x600
        this.load.image('bomb', '/phaser/assets/bomb.png'); // 14x14
        this.load.image('star', '/phaser/assets/star.png'); // 24x22
        this.load.image('ground', '/phaser/assets/platform.png'); // 400x32
    ...
    function create() {
        ...
        function hitBomb(player, bomb) {
            ...
            scoreText = this.add.text(280, 250, 'Game Over', {
                fontSize: '50px',
                color: '#fff',
                // fill: '#000',
    ...
    function update() {
        ...
        // up
        if(cursor.up.isDown) {
            if(player.body.touching.down) player.setVelocityY(-330); // 바닥에 닿았을 때 만 점프 가능
        }
    }
