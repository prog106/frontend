let Bookshelf = (function () {
    ...
    function shelfclass() {
        ...
        common.ax_fetch_get(url, function (res) {
            if (res.success) {
                ...
                document
                    .querySelector("select[name=shelfclass_name]")
    ...
    function getinfo() {
        ...
        common.ax_fetch_get(url, function (res) {
            if (res.success) {
                book_data = res.data;
                let shelfclass_name = document.querySelector(
                    "select[name=shelfclass_name]"
                );
                let shelfclass_value =
                    shelfclass_name.options[shelfclass_name.selectedIndex]
                        .value;
                let bhtml = book_data
                    .filter(function (item) {
                        return item.shelf_name.indexOf(shelfclass_value) != -1;
                    })
                    .map((item) => bookhtml(item))
                    .join("");
    ...
    function search() {
        ...
        let shelfclass_value =
            shelfclass_name.options[shelfclass_name.selectedIndex].value;

        let value = document.querySelector(
            "input[name=bookshelf_keyword]"
        ).value;
        document.querySelector(".remove_keyword").style.display = value
            ? "inline-block"
            : "none";
            ...
            .filter(function (item) {
                return (
                    item.shelf_name
                        ...
                        .toLowerCase()
                        .indexOf(
                            shelfclass_value.replace(/ /g, "").toLowerCase()
                        ) != -1
                );
            })
            .filter(function (item) {
    ...
    function bookhtml(item) {
        ...
        return `<li class="book_info">
            ...
            <div class="book_subinfo">
                <div class="book_title">${item.title.replace(
                    /<[^>]*>?/gm,
                    ""
                )}</div>
                <div class="book_status">${icon}</div>
                <div class="book_isbn">ISBN : <span>${item.isbn13}</span></div>
                <div class="book_author">저자 : <span>${item.authors
                    .split("|")
                    .join(", ")}</span></div>
                <div class="book_translator">번역 : <span>${item.translators
                    .split("|")
                    .join(", ")}</span></div>
                <div class="book_publisher">출판 : <span>${item.publisher
                    .split("|")
                    .join(", ")}</span></div>
                <div class="book_regdate">${item.regdate}</div>
                <div class="book_link"><a href="${
                    item.link
                }" target="_blank">책 상세보기</a></div>
                <div class="book_button">
                    ${
                        item.mybook
                            ? `<button class="move_myshelf" onclick="javascript:;">내 책꽂이에 있는 책입니다.</button>`
                            : `<button class="move_myshelf" onclick="Bookshelf.move_myshelf('${item.isbn13}', this);">내 책꽂이로 옮기기 +</button>`
                    }
                </div>
            </div>
        </li>`;
    ...
    function bookshelf_modal() {
        ...
        document
            .querySelector(".modal .book_search .close")
            .addEventListener("click", function () {
                setTimeout(function () {
        ...
        common.ax_fetch_put(url, form_data, function (res) {
            ...
            if (res.data.length > 0) {
                if (page == 1) {
                    ...
                    document.querySelector(".book_search_list").innerHTML = "";
                    shtml +=
                        '<li class="book_info">검색 결과 ( ' +
                        res.info.total_count +
                        " )</li>";
                }
                book_data = book_data.concat(res.data);
                res.data.forEach(function (v, k) {
                    let thumbnail = v.thumbnail.replace(
                        "type=m1&",
                        "type=m140&"
                    );
                    shtml += `<li class="book_info">
                        ...
                        <div class="book_subinfo">
                            <div class="book_title">${v.title.replace(
                                /<[^>]*>?/gm,
                                ""
                            )}</div>
                            <div class="book_isbn">ISBN : <span>${
                                v.isbn13
                            }</span></div>
                            <div class="book_author">저자 : <span>${v.authors
                                .split("|")
                                .join(", ")}</span></div>
                            <div class="book_translator">번역 : <span>${v.translators
                                .split("|")
                                .join(", ")}</span></div>
                            <div class="book_publisher">출판 : <span>${v.publisher
                                .split("|")
                                .join(", ")}</span></div>
                            <div class="book_regdate">${v.regdate}</div>
                            <div class="book_link"><a href="${
                                v.link
                            }" target="_blank">책 소개</a></div>
                            <div class="book_button">
                                <button class="add_book" onclick="Commonjs.add_bookshelf(${
                                    v.isbn13
                                }, this)">우리 가족 책장에 담기 +</button>
                            </div>
                        </div>
                    </li>`;
                });
                document.querySelector(".book_search_nodata").style.display =
                    "none";
                page++;
                if (!res.info.is_end)
                    shtml += `<li class="book_info more" onclick="Bookshelf.booksearch(${page})">더 보기 +</li>`;
                if (document.querySelector(".book_info.more"))
                    document.querySelector(".book_info.more").remove();
                document
                    .querySelector(".book_search_list")
                    .insertAdjacentHTML("beforeend", shtml);
                if (document.querySelector("input[name=keyword]:focus"))
    ...
    function add_bookshelf(isbn13, btn) {
        if (fetch) return false;
        fetch = true;
        let book = book_data.find(function (v) {
            if (v.isbn13 == isbn13) return true;
        });
        let url = "/bookshelf";
        let form_data = new FormData();
        form_data.append("book", JSON.stringify(book));
    ...
    function booksearchtext() {
        let remove_btn = document.querySelector(
            "form#search_book_form .remove_keyword"
        );
        let keyword = document.querySelector(
            "form#search_book_form input[name=keyword]"
        );
        if (keyword.value) {
